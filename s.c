...
char *scan_binlit(struct scan_token_st *tp, char *p, char *end) {
    ...
    if(*p == '0'){
        ...
        if(*p == 'b'){
        	p++;
        	curr = p;
        	while (scan_is_digit(*curr) && (curr != end)){
        	curr++;
        	len++;
        	}
        	tok = scan_read_token(tp,p,len,TK_BINLIT);
        	}
        	 if(*p == 'x'){
        	 	p++;
        		curr = p;
	        	while ((*curr >= 'a' && *curr <= 'f')  || scan_is_digit(*curr) || (*curr >= 'A' && *curr <= 'F')){
	        	curr++;
	        	len++;
	        	}
	        	tok = scan_read_token(tp,p,len,TK_HEXLIT);
        	}
         }
